...
while ((campos = leer_linea_csv(archivo, ',')) != NULL) {
    ...
    //strcpy(peli->genres, campos[10]);
    // Inserta la película en el mapa pelis_byid usando el ID como clave
    map_insert(pelis_byid, peli->id, peli);
    map_insertList(moviesByDirector, peli->director, peli);

  char *generos = strdup(campos[12]); // Duplicamos la cadena para evitar modificarla
  char *token = strtok(generos, ","); // Tokenizamos la cadena por comas
  while (token != NULL) {
  // Creamos una copia de la película para cada género
  Film *copy = (Film *)malloc(sizeof(Film));
    memcpy(copy, peli, sizeof(Film)); // Copia la película
    list_pushBack(copy->genres, token); // Inserta el género en la copia
    map_insertList(pelis_bygen, token, copy); // Inserta la copia en el mapa con el género como clave
    token = strtok(NULL, ",");
    }
  free(generos);
}
  ...
  while (pair != NULL) {
    ...
    pair = map_next(pelis_bygen); // Avanza al siguiente par en el mapa
  }
}*/
...
void cargar_peliculas(Map *pelis_byid, Map *moviesByDirector, Map *pelis_bygen) {
    ...
    while ((campos = leer_linea_csv(archivo, ',')) != NULL) {
        ...
        // Separa los géneros y los inserta en el mapa pelis_bygen
        char *generos = strdup(campos[12]); // Duplicamos la cadena para evitar modificarla
        char *token = strtok(generos, ","); // Tokenizamos la cadena por comas
        while (token != NULL) {
            // Creamos una copia de la película para cada género
        ...
        free(generos); // Liberamos la memoria asignada a la cadena duplicada
    }
    fclose(archivo); // Cierra el archivo después de leer todas las líneas
}
