...
void map_insert2(Map *map, void *key, void *value) {
  MapPair *pair = map_search(map, key);
  if (pair == NULL) {
    // Si la clave no existe en el mapa, creamos una nueva lista y la insertamos
    List *list = list_create();
    list_pushBack(list, value);
    MapPair *newPair = (MapPair *)malloc(sizeof(MapPair));
    newPair->key = key;
    newPair->value = list;
    list_pushBack(map->ls, newPair);
  } else {
    // Si la clave ya existe, obtenemos la lista asociada y añadimos el valor
    List *list = (List *)pair->value;
    list_pushBack(list, value);
  }
}
